...
/** 
 * [알리고 알림톡 설정] (보안상 서버 처리 권장; 여기서는 예시로 클라이언트에서)
 */
...
/** 
 * [추가] "무통장 입금확인" 알림톡 발송
 *   row = {예약번호, 예약자, 전화번호, 이용객실, 이용기간, 수량, 옵션, 총이용인원, 입실시간, 결제금액}
 */
function sendAlimtalkForDeposit(row) {
  // 1) 숙박 vs 당일
  const isStay = row.이용기간.includes('~'); // "~" 있으면 숙박
  let tplCode, tplText;
  if(isStay) {
